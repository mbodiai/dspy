...
class Image(BaseModel):
    ...
    encoding: Literal["png", "jpeg", "jpg", "bmp", "gif"] = Field(
        "jpeg", exclude=False
    )
    ...
    def supports(cls, arg: SupportsImage) -> bool:
        return (
            isinstance(arg, (np.ndarray, PILImage))
            or Path(arg).exists()
            or arg.startswith("data:image")
            or urlparse(arg).scheme
        )
    ...
    def validate_kwargs(cls, values) -> dict:
        ...
        if len(provided_fields) > 1:
            raise ValueError(
                "Multiple image sources provided; only one is allowed."
            )
        ...
        if "path" in values:
            ...
            validated_values.update(
                cls.from_pil(image, validated_values["encoding"])
            )
        ...
        # Convert to PIL image and populate other fields
            ...
            validated_values.update(
                cls.from_pil(values["pil"], validated_values["encoding"])
            )

        # If 'base64' is provided, decode and populate other fields
        elif "base64" in values:
            validated_values.update(
                cls.from_base64(values["base64"], validated_values["encoding"])
            )
        ...
        if validated_values["encoding"] not in [
            "png",
            "jpeg",
            "jpg",
            "bmp",
